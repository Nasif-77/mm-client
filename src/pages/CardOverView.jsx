...
function CardOverView() {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            try {
                ...
                const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/api/admin/${reqType === 'mentors' ? 'staffs' : reqType}/${id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`
                    }
                })
                if (isMounted) response.data.data?.isDeleted === false ? setUserData(response.data.data) : navigate(`/${location.pathname.split('/')[1]}`)
            } catch (error) {
    ...
    return (
        <div>
            {userData?.isDeleted === false ? <div>
                {userRole !== 'students' &&
                    <>
                        {type === 'inquiries' ? <InquiryCard userData={userData} /> : null}
                ...
                {type === 'assignments' ? <AssignmentCard userData={userData} /> : null}
            </div> : <p>{response}</p>}
        </div>
    )
}
